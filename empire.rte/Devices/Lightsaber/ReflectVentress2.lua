  ...
  if MovableMan:IsActor(self.parent) and self.RootID == self.parent.ID then
	...
	if not self.ReflectTimer:IsPastSimMS(310 - (20 * self.parent.Sharpness)) then
		...
		if self.parent:GetController():IsState(13) then
			...
			self.Pos = self.Pos + Vector(0,-3 * self.negatore):RadRotate(self.parent:GetAimAngle(true));
		else
			self.RotAngle = angle - .8 * self.negatore;
		end
	end
	...
	if self.SpinTimer:IsPastSimMS(535 - (20 * self.parent.Sharpness)) and self.ReflectTimer:IsPastSimMS(370) and self.BlockAngle == 0 then
...
--SHIELD
   ...
   if particle.HitsMOs == false then
	...
	if pardist.Magnitude < 22 and particle.PresetName == "Lightsaber Killing B" then
...
--GRAZE BLOCK
	if pardist.Magnitude < 22 and particle.PresetName == "Lightsaber Killing C" and particle.Team ~= self.Team then
		if self.ammoCounter > 1 then
...
if self.ReflectTimer:IsPastSimMS(410 - (20 * self.parent.Sharpness)) then
	local rayL = 0;  
