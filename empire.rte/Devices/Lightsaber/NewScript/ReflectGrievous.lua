...
function Update(self)
  ...
  if MovableMan:IsActor(self.parent) and self.RootID == self.parent.ID then
	...
	if not self.ReflectTimer:IsPastSimMS(310 - (20 * self.parent.Sharpness)) then
		...
		self.RotAngle = angle -.25 * self.negatore;
	end
	...
	if self.SpinTimer:IsPastSimMS(435 - (20 * self.parent.Sharpness)) and self.ReflectTimer:IsPastSimMS(350) and self.BlockAngle == 0 then
		...
		self.anglemodifier = angmod - 0.28 * posneg
		self.Magazine.Frame = 14;
...
--SHIELD
   ...
   if particle.HitsMOs == false then
	if pardist.Magnitude < 18 and particle.PresetName == "Lightsaber Killing B" and particle.Team ~= self.Team then
		if self.ammoCounter > 1 then
--	particle.ToDelete = true;
	...
	self.parent:AddAbsForce((particle.Vel) * 0.35, strikePoint);
...
--GRAZE BLOCK
	if pardist.Magnitude < 18 and particle.PresetName == "Lightsaber Killing C" and particle.Team ~= self.Team then
		if self.ammoCounter > 1 then
...
if self.ReflectTimer:IsPastSimMS(440 - (20 * self.parent.Sharpness)) then
	local rayL = 0;  
	local vect = Vector(18,0);

		vect = Vector(-16 * self.negatore,-16);
	...
	self.hitnum = self.hitnum + 1;
		...
		while i < 6 do
			...
			self.ammoCounter = self.ammoCounter + 5;
			self.hitnum = 0;
			self.ReflectTimer:Reset();
...
--END SWING
		...
		if self.rechargeTimer:IsPastSimMS(65 - sharpness * 5) and self.ammoCounter < sharpness * 8 then
			self.rechargeTimer:Reset();
			self.ammoCounter = self.ammoCounter + 1;
			self.anglemodifier = 0;
		end
