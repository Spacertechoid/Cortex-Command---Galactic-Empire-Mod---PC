...
function Update(self)
  ...
  if MovableMan:IsActor(self.parent) and self.RootID == self.parent.ID then
	...
	if not self.SpinTimer:IsPastSimMS(2000) then
	 if self.ReflectTimer:IsPastSimMS(470 - (20 * self.parent.Sharpness)) then
		angmod = math.sin(self.ReflectTimer.ElapsedSimTimeMS / 200) / 1.9;
		--self.Magazine.Frame = 8

		if angmod == 0 then
			local soundfx = CreateAEmitter("Lightsaber Blade Sound");
			soundfx.Pos = self.Pos;
			MovableMan:AddParticle(soundfx);
		end
	 end
	else
	 if self.ReflectTimer:IsPastSimMS(150) and self.BlockAngle == 0 then
	...
	if posneg == 1 then
		...
		self.anglemodifier = angmod + 0.27 * posneg
	...
	self.RotAngle = angle + (.4 + angmod ) * self.negatore;
	self.Pos = self.Pos + Vector(angmod * 0.5 * self.negatore,0):RadRotate(self.RotAngle);
...
--SHIELD
   ...
   if particle.HitsMOs == false then
	...
	if pardist.Magnitude < 25 and particle.PresetName == "Lightsaber Killing B" and particle.Team ~= self.Team then
		if self.ammoCounter > 1 then
--	particle.ToDelete = true;
	...
	self.parent:AddAbsForce((particle.Vel) * 0.6, strikePoint);
...
--GRAZE BLOCK
	if pardist.Magnitude < 25 and particle.PresetName == "Lightsaber Killing C" and particle.Team ~= self.Team then
		if self.ammoCounter > 1 then
...
--END GRAZE BLOCK
	...
	local strikePoint = Vector(0,-1);
	self.parent:AddAbsForce((particle.Vel) * 0.5, strikePoint);
...
if self.ReflectTimer:IsPastSimMS(370 - (20 * self.parent.Sharpness)) then
	local rayL = 0;  
	...
	self.hitnum = self.hitnum + 1;
		...
		while i < 6 do
			...
			MovableMan:AddParticle(soundfx);
				...
				local part1 = 120/(root.Mass)
				ToActor(root).Health = ToActor(root).Health - part1;

	self.parent:SetWhichMOToNotHit(root,279)
	root:SetWhichMOToNotHit(self.parent,279)

					local vect3 = Vector(-26,0);
						vect3 = Vector(-26 * self.negatore,0);
	
					root:AddAbsForce(vect3 * 3 * root.Mass * self.parent.Sharpness, SceneMan:GetLastRayHitPos());
		end
...
--BLOCKHIT
	if self.ammoCounter > 1 and not self.ReflectTimer:IsPastSimMS(70) then
...
--END SWING
		...
		if self.rechargeTimer:IsPastSimMS(65 - sharpness * 5) and self.ammoCounter < sharpness * 35 then
			self.rechargeTimer:Reset();
			self.ammoCounter = self.ammoCounter + 1;
			self.anglemodifier = 0;
			self.hitnum = 0;
