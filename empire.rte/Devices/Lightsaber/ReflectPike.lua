...
function Update(self)
  ...
  if MovableMan:IsActor(self.parent) and self.RootID == self.parent.ID then
	...
	if self.SpinTimer:IsPastSimMS(935 - (20 * self.parent.Sharpness)) and self.ReflectTimer:IsPastSimMS(350) and self.BlockAngle == 0 then
...
--SHIELD
   ...
   if particle.HitsMOs == false then
	...
	if pardist.Magnitude < 27 and particle.PresetName == "Lightsaber Killing B" then
		if self.ammoCounter > 1 then
...
--GRAZE BLOCK
	if pardist.Magnitude < 27 and particle.PresetName == "Lightsaber Killing C" and particle.Team ~= self.Team then
		if self.ammoCounter > 1 then
...
--END GRAZE BLOCK
	...
	if particle.ClassName == "MOPixel" and pardist.Magnitude < 30 and particle.Vel.Magnitude > 30 and particle.PresetName ~= "Lightsaber Killing B" then
		if self.ammoCounter > 1 then
	local angle = SceneMan:ShortestDistance(self.Pos,particle.Pos, true).AbsRadAngle
	...
	elseif particle.ClassName ~= "MOPixel" and pardist.Magnitude < 25 and particle.Vel.Magnitude > 30 and particle.PresetName ~= "Lightsaber Killing B"  and particle.Mass < 50 then
	particle.ToDelete = true;
...
if self.ReflectTimer:IsPastSimMS(475 - (20 * self.parent.Sharpness)) then
	local rayL = 0;  
	local vect = Vector(20,0);
	...
	self.hitnum = self.hitnum + 1;
		...
		while i < 8 do
			...
			self.ammoCounter = self.ammoCounter + 6;
			self.hitnum = 0;
			self.ReflectTimer:Reset();
...
--BLOCKHIT
	if self.ammoCounter > 1 and not self.ReflectTimer:IsPastSimMS(40) then
	self.ammoCounter = self.ammoCounter - 0.45;
	local rayL = 0;  
...
--END SWING
		...
		if self.rechargeTimer:IsPastSimMS(75 - sharpness * 5) and self.ammoCounter < sharpness * 30 then
			self.rechargeTimer:Reset();
			self.ammoCounter = self.ammoCounter + 1;
			self.anglemodifier = 0;
			self.hitnum = 0;
