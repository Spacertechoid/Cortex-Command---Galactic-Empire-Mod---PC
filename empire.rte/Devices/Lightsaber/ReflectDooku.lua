...
function Update(self)
  ...
  if MovableMan:IsActor(self.parent) and self.RootID == self.parent.ID then
	...
	if not self.ReflectTimer:IsPastSimMS(290 - (20 * self.parent.Sharpness)) then
		...
		self.Magazine.Frame = math.random(0,3);
			...
			if blockang ~= 0 then
				...
				if self.BlockAngle < -0.1 and self.BlockAngle > 0.1 then
					local rand = math.random(0,4)
					if rand < 2 then
					...
					if self.BlockAngle > -0.05 and self.BlockAngle < 0.05 then
						self.BlockAngle = 0;
					end
	...
	if self.parent:GetController():IsState(13) then
		...
		local height = self.StanceOffset.Y;

		if height > -1 then
			height = -1
		end

		self.RotAngle = angle - .375 * height * self.negatore;
	...
	if self.SpinTimer:IsPastSimMS(330- (10 * self.parent.Sharpness)) and self.ReflectTimer:IsPastSimMS(265) and self.BlockAngle == 0 then
		angmod = self.anglemodifier

		if angmod == 0 then
			local soundfx = CreateAEmitter("Sith Lightsaber Swing");
			soundfx.Pos = self.Pos;
			MovableMan:AddParticle(soundfx);
		end

		if self.ReflectTimer:IsPastSimMS(350) and not self.ReflectTimer:IsPastSimMS(1550) and not self.parent:GetController():IsState(13) then
			if self.randblock < 0 then
				posneg = -1;
			end
		end

		self.anglemodifier = angmod - 0.38 * posneg;



	if posneg == 1 then
		self.Magazine.Frame = 15;
		if angmod <= -(math.pi * 4) then
			self.anglemodifier = 0;
			angmod = 0;
...
--SHIELD
   for particle in MovableMan.Particles do
	...
	if pardist.Magnitude < 35 then
	if particle.Team ~= self.Team then

   if particle.HitsMOs == false then
	if pardist.Magnitude < 35 and particle.PresetName == "Lightsaber Killing B" and particle.Team ~= self.Team then
		if self.ammoCounter > 1 then
--	particle.ToDelete = true;
	...
	self.randblock = math.random(-2,2);
...
--GRAZE BLOCK
	if pardist.Magnitude < 35 and particle.PresetName == "Lightsaber Killing C" and particle.Team ~= self.Team then
		if self.ammoCounter > 1 then
...
--END GRAZE BLOCK
		...
		if pardist.Magnitude < 25 and particle.PresetName == "Lightning Particle Null" and particle.Team ~= self.Team then
			particle.ToDelete = true;
	...
	if particle.ClassName == "MOPixel" and pardist.Magnitude < 33 and particle.Vel.Magnitude > 20 and particle.PresetName ~= "Lightsaber Killing B" and particle.Team ~= self.Team then
		if self.ammoCounter > 1 then
	local angle = SceneMan:ShortestDistance(self.Pos,particle.Pos, true).AbsRadAngle
	...
	local soundfx = CreateAEmitter("Sith Lightsaber Short Swing");
	soundfx.Pos = particle.Pos;
	MovableMan:AddParticle(soundfx);
	...
	local strikePoint = Vector(0,-1);
	self.parent:AddAbsForce((particle.Vel) * 0.1, strikePoint);
	...
	elseif particle.ClassName ~= "MOPixel" and pardist.Magnitude < 30 and particle.Vel.Magnitude > 30 and particle.PresetName ~= "Lightsaber Killing B" and particle.Mass < 50 and particle.Team ~= self.Team then
...
if self.ReflectTimer:IsPastSimMS(290 - (20 * self.parent.Sharpness)) then
	local rayL = 0;  
	local vect = Vector(18,0);

		vect = Vector(-20 * self.negatore,-20);
	...
	self.hitnum = self.hitnum + 1;
		...
		while i < 8 do
			...
			self.ammoCounter = self.ammoCounter + 4;
			self.hitnum = 0;
			self.ReflectTimer:Reset();
	...
	root:SetWhichMOToNotHit(self.parent,349)
					...
					root:AddAbsForce((vect3 * root.Mass * self.parent.Sharpness):RadRotate(self.RotAngle), SceneMan:GetLastRayHitPos());
		end
			end
...
--BLOCKHIT
	if self.ammoCounter > 1 and not self.ReflectTimer:IsPastSimMS(60) then
		self.ammoCounter = self.ammoCounter - 0.35;
	local rayL = 0;  
	local vect = Vector(18,0);

		vect = Vector(-19 * self.negatore,-19);
...
--END SWING
		...
		if self.rechargeTimer:IsPastSimMS(65 - sharpness * 5) and self.ammoCounter < sharpness * 30 then
			self.rechargeTimer:Reset();
			self.ammoCounter = self.ammoCounter + 1;
			self.anglemodifier = 0;
			self.hitnum = 0;
