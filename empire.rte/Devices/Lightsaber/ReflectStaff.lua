...
function Update(self)
  ...
  if MovableMan:IsActor(self.parent) and self.RootID == self.parent.ID then
	...
	if self.parent:GetController():IsState(13) then
		...
		self.RotAngle = angle + .6 * self.negatore;
	else
		local angle = self.RotAngle;
		self.RotAngle = angle + 1.26 * self.negatore;
	end
	...
	if self.SpinTimer:IsPastSimMS(580- (10 * self.parent.Sharpness)) and self.ReflectTimer:IsPastSimMS(330) and self.BlockAngle == 0 then
		...
		self.anglemodifier = angmod - 0.3 * posneg;
					local rand = math.random(0,4)
					if rand < 2 then
						self.Magazine.Frame = 14
					else
	...
	local angle = self.RotAngle;
	self.RotAngle = angle - (.35 + angmod )* self.negatore;
...
--SHIELD
   for particle in MovableMan.Particles do
	...
	if pardist.Magnitude < 30 then
	if particle.Team ~= self.Team then
   if particle.HitsMOs == false then

	if pardist.Magnitude < 28 and particle.PresetName == "Lightsaber Killing B" and particle.Team ~= self.Team then
		if self.ammoCounter > 1 then
...
--GRAZE BLOCK
	if pardist.Magnitude < 28 and particle.PresetName == "Lightsaber Killing C" and particle.Team ~= self.Team then
		if self.ammoCounter > 1 then
...
--END GRAZE BLOCK
	...
	if particle.ClassName == "MOPixel" and pardist.Magnitude < 30 and particle.Vel.Magnitude > 30 and particle.PresetName ~= "Lightsaber Killing B" and particle.Team ~= self.Team then
		if self.ammoCounter > 1 then
	local angle = SceneMan:ShortestDistance(self.Pos,particle.Pos, true).AbsRadAngle
...
if self.ReflectTimer:IsPastSimMS(310 - (20 * self.parent.Sharpness)) then
	local rayL = 0;  
	local vect = Vector(18,0);


	vect = Vector(23,23 * self.negatore);
	...
	vect = Vector(-18,-18 * self.negatore);
	--vect = Vector(-19,-19 * self.negatore);
	vect = vect:RadRotate(self.RotAngle); 

	rayL2 = SceneMan:CastObstacleRay(Vector(self.Pos.X,self.Pos.Y),vect,Vector(0,0),Vector(0,0),self.parent.ID,self.Team,0,1);  

	if rayL>0 or rayL2> 0 then  
		...
		if MovableMan:IsActor(root) then
			...
			if hitMO and MovableMan:IsActor(root) and ToActor(root).Health > 0 then
				local angle = SceneMan:ShortestDistance(self.Pos,root.Pos, true).AbsRadAngle
					...
					self.randblock = math.random(-6,6);
	...
	self.hitnum = self.hitnum + 1;
		...
		while i < 7 do
			...
			MovableMan:AddParticle(soundfx);
				...
				local part1 = 110/(root.Mass)
				ToActor(root).Health = ToActor(root).Health - part1;

	self.parent:SetWhichMOToNotHit(root,249)
	root:SetWhichMOToNotHit(self.parent,249)

					local vect3 = Vector(-26,0);
						vect3 = Vector(-26 * self.negatore,0);
	
					root:AddAbsForce(vect3 * root.Mass * self.parent.Sharpness, SceneMan:GetLastRayHitPos());
		end
			end
...
--BLOCKHIT
	if self.ammoCounter > 1 and not self.ReflectTimer:IsPastSimMS(80) then
	self.ammoCounter = self.ammoCounter - 0.25;
	local rayL = 0;  
	local vect = Vector(18,0);
	...
	vect = Vector(-15,-15 * self.negatore);
	vect = vect:RadRotate(self.RotAngle); 
...
--END SWING
		...
		if self.rechargeTimer:IsPastSimMS(75 - sharpness * 5) and self.ammoCounter < sharpness * 40 then
			self.rechargeTimer:Reset();
			self.ammoCounter = self.ammoCounter + 1;
			self.anglemodifier = 0;
			self.hitnum = 0;
