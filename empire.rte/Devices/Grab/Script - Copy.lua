			...
			if self.target.ClassName == "ADoor" then
					for i = 0, MovableMan:GetMOIDCount() do
						if MovableMan:GetRootMOID(i) == self.target.ID then
							...
							self.target.Vel = Vector(40 + 30 * (275/object.Mass),0):RadRotate(self.parent:GetAimAngle(true));
							object:AddAbsForce(Vector(50 * 150 * self.parent.Sharpness,0):RadRotate(self.parent:GetAimAngle(true)), SceneMan:GetLastRayHitPos());
						end
					end
			end
...
if self.ammoCounter > 1 then
	...
	if self.parent:GetController():IsState(Controller.HOLD_LEFT) then
		local groundray = SceneMan:CastStrengthRay(self.target.Pos,Vector(-15,0),0,terrcheck,1,0,true);
	   if groundray == false then
		self.target.Vel = Vector(-25,0);
	...
	elseif self.parent:GetController():IsState(Controller.HOLD_UP) then
		local groundray = SceneMan:CastStrengthRay(self.target.Pos,Vector(0,-15),0,terrcheck,1,0,true);
	   if groundray == false then
		self.target.Vel = Vector(0,-25);
		self.ammoCounter = self.ammoCounter - 0.15;
	   end
	elseif self.parent:GetController():IsState(Controller.HOLD_DOWN) then
	   ...
	   if groundray == false then
		...
		self.target.Vel = self.target.Vel * 0.85;
	...
	--FIND TARGET
		targetID1 = SceneMan:CastMORay(self.Pos, Vector(200 + self.parent.Sharpness * 10,0):RadRotate(self.parent:GetAimAngle(true)), self.parent.ID, self.Team, 0, true,0)
		targetMO1 = nil
...
elseif self.parent.Vel.Magnitude > 20 then

	--FORCE STOP
	self.parent.Vel = Vector(0,0);

	if self.ammoCounter > 1 then
		self.ammoCounter = self.ammoCounter - 0.06;
	end
	...
	if self.target then
      ...
      if MovableMan:IsActor(self.target) and self.ammoCounter > 20 and self.lifttimer:IsPastSimMS(520 - 50 * self.parent.Sharpness) then

	local throwvel = 20 + (self.parent.Sharpness * (395/self.target.Mass));
			...
			if actor.ID ~= self.target.ID and actor.Team ~= self.Team then
			 if self.ammoCounter > 3 then
				local dist = SceneMan:ShortestDistance(self.target.Pos, actor.Pos, true).Magnitude
					if dist < (20 + self.parent.Sharpness * 6)then
...
--	end
	...
	self.target:SetWhichMOToNotHit(self.parent,1549)
		...
		if self.rechargeTimer:IsPastSimMS(150 - sharpness * 15) and self.ammoCounter < sharpness * 50 then
			self.rechargeTimer:Reset();
     ...
     if MovableMan:IsActor(actor) then
	self.parent = ToActor(actor);
	self.totalAmmo = self.parent.Sharpness * 50;
     end
end
