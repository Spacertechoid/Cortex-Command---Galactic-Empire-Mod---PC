function Create(self)
	...
	local magseal = CreateMOPixel("WESTAR Magnetic Seal 2");
	magseal.Pos = self.Pos + Vector(0.7,0.7):RadRotate(self.Vel.AbsRadAngle);
	magseal.Vel = self.Vel;
	magseal.Team = self.Team;
	magseal.IgnoresTeamHits = true;
	MovableMan:AddParticle(magseal);
...
local terrcheck = Vector(0,0);
	...
	if self.Timer:IsPastSimMS(400) == true then
		self.ToDelete = true;
	end
