function Create(self)
	...
	local magseal = CreateMOPixel("E23 Magnetic Seal");
	...
	local magseal = CreateMOPixel("E23 Magnetic Seal B");
	magseal.Pos = self.Pos + Vector(-0.5,-0.5):RadRotate(self.Vel.AbsRadAngle);
	magseal.Vel = self.Vel;
	magseal.Team = self.Team;
...
local terrcheck = Vector(0,0);
	if SceneMan:CastStrengthRay(self.Pos,Vector(self.Vel.X,self.Vel.Y):SetMagnitude(35),0,terrcheck,0,0,true) then
      		self.ToDelete = true;
  	end
